...
{
	/// <summary>
	///     A utility class to assist with NAT traversal; specifically, ports may be forwarded through a NAT router to support
	///     inbound connections. This class uses UPnP.
	/// </summary>
	public static class NatHelper
	{
		private const string BroadcastPacket =
		...
		private const string ControlUrlPath =
			"//tns:service[tns:serviceType=\"urn:schemas-upnp-org:service:WANIPConnection:1\"]/tns:controlURL";
		...
		{
			get { return !string.IsNullOrEmpty(_controlUrl); }
		}
		...
		public static IAsyncResult BeginDiscover(AsyncCallback callback = null, object state = null)
		{
			...
			var outBuf = Encoding.ASCII.GetBytes(BroadcastPacket);
			...
			ThreadPool.QueueUserWorkItem((o) =>
			{
				...
				while (--tries >= 0)
				{
					try
					{
						...
						while (true)
						{
							var length = sock.Receive(inBuf);
							var data = Encoding.ASCII.GetString(inBuf, 0, length).ToLowerInvariant();

							var match = ResponseLocation.Match(data);
							if (match.Success &&
								match.Groups["location"].Success)
							{
								Debug.WriteLine("Found UPnP device at " + match.Groups["location"]);
								var controlUrl = GetServiceUrl(match.Groups["location"].Value);
								if (!string.IsNullOrEmpty(controlUrl))
								{
									_controlUrl = controlUrl;
									Debug.WriteLine("Found control URL at " + _controlUrl);
		...
		public static IAsyncResult BeginAddForwardingRule(int port, ProtocolType protocol, string description,
			AsyncCallback callback = null, object state = null)
		{
			...
			{
				address =
					Dns.GetHostAddresses(Dns.GetHostName()).Where((ip) => ip.AddressFamily == AddressFamily.InterNetwork).First();
			}
			catch
			{
			...
			ThreadPool.QueueUserWorkItem((o) =>
			{
				try
				{
					...
					ar.IsSuccessful = true;
					Debug.WriteLine("Started forwarding port " + port);
				}
				catch (Exception ex)
				{
					Debug.WriteLine(ex.ToString());
		...
		/// <summary>
		///     End the port forwarding operation. If the operation is not yet completed, this method will block until it
		///     completes.
		/// </summary>
		...
		public static IAsyncResult BeginDeleteForwardingRule(int port, ProtocolType protocol, AsyncCallback callback = null,
			object state = null)
		{
			if (!IsAvailable)
			{
			...
			ThreadPool.QueueUserWorkItem((o) =>
			{
				try
				{
					...
					ar.IsSuccessful = true;
					Debug.WriteLine("Stopped forwarding port " + port);
				}
				catch (Exception ex)
				{
					Debug.WriteLine(ex.ToString());
		...
		private static XDocument SoapRequest(string url, XElement content, string function)
		{
			...
			req.Method = "POST";
			var buf = Encoding.UTF8.GetBytes(doc.ToString());
		...
		private static string GetServiceUrl(string url)
		{
			...
			{
				var uri = new Uri(url);
				return string.Format("http://{0}:{1}{2}", uri.Host, uri.Port, node.Value);
			}
