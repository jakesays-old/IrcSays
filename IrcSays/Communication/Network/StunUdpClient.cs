...
{
	/// <summary>
	///     Provides facilities for creating a UDP socket binding where the public endpoint for that binding is known. This
	///     endpoint can be communicated to
	...
	public class StunUdpClient
	{
		...
		///     The list of stun servers to query. This may be a list of hostnames or IP addresses and may
		///     also specify a port separated by a colon.
		/// </param>
		...
		/// <summary>
		///     Begins an asynchronous operation to construct a UDP client and query a STUN server for the corresponding public
		///     endpoint to be used with that client.
		/// </summary>
				...
				while (true)
				{
					if (arr.AsyncWaitHandle.WaitOne(Math.Max(1, StunTimeout - (int) (DateTime.Now - startTime).TotalMilliseconds)))
					{
						...
						if (response.Length > HeaderLength &&
							response[0] == 0x01 &&
							response[1] == 0x01 && // MessageType = Response
							response.Skip(4).Take(4).SequenceEqual(StunCookie) && // Magic Cookie
							response.Skip(8).Take(12).SequenceEqual(idBytes)) // Transaction ID
						{
							var idx = HeaderLength;
							while (idx + 24 < response.Length)
							{
								var attrType = (ushort) response[idx] << 8 | (ushort) response[idx + 1];
								var attrLength = (ushort) response[idx + 2] << 8 | (ushort) response[idx + 3];
								if (attrType == 0x01 &&
									attrLength >= 8) // MAPPED_ADDRESS
								{
									var port = (ushort) response[idx + 6] << 8 | (ushort) response[idx + 7];
