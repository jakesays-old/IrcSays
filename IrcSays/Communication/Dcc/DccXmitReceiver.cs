...
namespace IrcSays.Communication.Dcc
{
	...
	public sealed class DccXmitReceiver : DccOperation
	{
		...
		private readonly byte[] _timeStampBytes = new Byte[4];
		...
		/// <summary>
		///     Gets or sets a value indicating whether a resume will be forced even if the transferred file does not appear to be
		...
		protected override void OnReceived(byte[] buffer, int count)
		{
			...
			if (!_isTransferring)
			{
				...
				var resumeBytes = BitConverter.GetBytes((int) 0);
				if (_handshakeBytesReceived >= 4)
				{
					_timeStamp = IPAddress.NetworkToHostOrder(BitConverter.ToInt32(_timeStampBytes, 0));
					if (_fileInfo.Exists &&
						_fileInfo.Length < int.MaxValue &&
						(ForceResume ||
						(_timeStamp > 0 && _timeStamp == (int) (_fileInfo.LastWriteTimeUtc - new DateTime(1970, 1, 1)).TotalSeconds)))
					{
						...
						BytesTransferred = _fileInfo.Length;
					}
					else
					{
						var i = 1;
