...
namespace IrcSays.Ui
{
	public partial class ChatWindow : Window
	{
		...
		{
			var page = new FileControl.FileControl(session, target, DccMethod.Xmit);
			App.Create(session, page, true);
		...
		{
			var page = new FileControl.FileControl(session, target, DccMethod.Send);
			App.Create(session, page, true);
			page.StartSend(file, (port) =>
			{
				if (port > 0)
