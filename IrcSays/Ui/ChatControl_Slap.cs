...
namespace IrcSays.Ui
{
	public partial class ChatControl : ChatPage
	{
		...
		private static readonly Random _slapRandom = new Random((int) DateTime.Now.Ticks);
		...
		private void ExecuteSlap(object sender, ExecutedRoutedEventArgs e)
		{
			...
			if (IsConnected)
			{
				Session.SendCtcp(Target, new CtcpCommand("ACTION", slapString.Split(' ')), false);
				Write("Own", string.Format("{0} {1}", Session.Nickname, slapString));
			}
		}
	}
}